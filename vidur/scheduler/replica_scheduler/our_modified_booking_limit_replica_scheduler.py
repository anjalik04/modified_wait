...
class OurModifiedBookingLimitReplicaScheduler(BaseReplicaScheduler):
    def __init__(self, *args, **kwargs):
        ...
        self.arrival_times = {pt["type"]: [] for pt in self.prompt_types}
        self.inter_arrival_data = {pt["type"]: [] for pt in self.prompt_types}
        self.rate_estimators = {pt["type"]: OnlineRateEstimator(alpha=0.05) for pt in self.prompt_types}
        self.smoothed_rates = {pt["type"]: [] for pt in self.prompt_types}
        self.final_rate_history = {pt["type"]: [] for pt in self.prompt_types}
        self.final_rate_running_sum = {pt["type"]: 0.0 for pt in self.prompt_types}
        self.final_rate_count = {pt["type"]: 0 for pt in self.prompt_types}
    ...
    def add_request(self, request: Request) -> None:
        ...
        print(self.num_arrival_requests)
        print(self)

        prompt_type = request.prompt_type
        now = request.arrived_at
        # print("helo")
        if prompt_type not in self.arrival_times:
            return
        # print("helo")
        if self.arrival_times[prompt_type]:
            delta = now - self.arrival_times[prompt_type][-1]
            self.inter_arrival_data[prompt_type].append(delta)

            # Use recent 10 deltas to compute local mean
            recent_deltas = self.inter_arrival_data[prompt_type][-10:]
            mean_delta = np.mean(recent_deltas) if recent_deltas else delta

            # Use this moving average for EMA rate update
            rate = self.rate_estimators[prompt_type].update(mean_delta)

            # Smooth and finalize rate
            raw_rates = self.rate_estimators[prompt_type].rates
            if len(raw_rates) >= 101:
                smoothed = savgol_filter(raw_rates, window_length=101, polyorder=3)
                # s_smoothed = savgol_filter(smoothed, window_length=101, polyorder=3)
                self.smoothed_rates[prompt_type] = smoothed
                final_rate = smoothed[-1]
            else:
                final_rate = rate

            # Update arrival_rate in config
            # self.final_rate_history[prompt_type].append(final_rate)

            # global_avg_final_rate = np.mean(self.final_rate_history[prompt_type])
            self.final_rate_running_sum[prompt_type] += final_rate
            self.final_rate_count[prompt_type] += 1

            # Compute average in O(1)
            global_avg_final_rate = (self.final_rate_running_sum[prompt_type] / self.final_rate_count[prompt_type])
            for pt in self.prompt_types:
                if pt["type"] == prompt_type:
                    pt["arrival_rate"] = global_avg_final_rate

            self.booking_limits_per_type = self.calculate_booking_limits()


        self.arrival_times[prompt_type].append(now)
        # print("helo")
        ...
        if self.num_arrival_requests == self.total_num_requests:
            print("Going through if statement")
    ...
    def on_batch_end(self, batch: Batch) -> None:
        ...
        print(self._num_running_batches)
