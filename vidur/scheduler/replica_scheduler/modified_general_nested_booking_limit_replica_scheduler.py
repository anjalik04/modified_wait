...
class ModifiedGeneralizedNestedBookingLimitReplicaScheduler(BaseReplicaScheduler):
    def __init__(self, *args, **kwargs):
        ...
        self.arrival_logs = {pt["type"]: [] for pt in self.prompt_types}
        self.inter_arrival_times = {pt["type"]: [] for pt in self.prompt_types}
        self.log_estimates = {pt["type"]: [] for pt in self.prompt_types}
        self.filtered_rates = {pt["type"]: [] for pt in self.prompt_types}
        self.transient_buffer = {pt["type"]: [] for pt in self.prompt_types}
        self.predicted_arrival_rates = {pt["type"]: [] for pt in self.prompt_types}
        self.smoothing_window = 10

        self.nested_booking_limits, self.segments = self.calculate_nested_booking_limits()

    def _confidence_interval(self, point: float) -> Tuple[float, float]:
        return (point - 0.5 * sqrt(point), point + 0.5 * sqrt(point))

    def _predict_and_store_log_rate(self, pt_type: str):
        timestamps = self.arrival_logs[pt_type]
        if len(timestamps) < 2:
            return

        new_interarrival = timestamps[-1] - timestamps[-2]
        self.inter_arrival_times[pt_type].append(new_interarrival)

        window = self.inter_arrival_times[pt_type][-self.smoothing_window:]
        avg_gap = mean(window)
        log_val = -log(avg_gap)
        self.log_estimates[pt_type].append(log_val)

        filtered = self.filtered_rates[pt_type]
        if not filtered:
            filtered.append(log_val)
        else:
            prev_val = filtered[-1]
            low, high = self._confidence_interval(prev_val)
            if low <= log_val <= high:
                filtered.append(prev_val)
            else:
                mid_val = mean([prev_val, log_val])
                low, high = self._confidence_interval(mid_val)
                if low <= prev_val <= high and low <= log_val <= high:
                    filtered.append(mid_val)
                elif len(self.transient_buffer[pt_type]) == 10:
                    buffer_avg = mean(self.transient_buffer[pt_type])
                    low, high = self._confidence_interval(buffer_avg)
                    if low <= log_val <= high:
                        filtered.append(buffer_avg)
                        self.transient_buffer[pt_type] = []
                    else:
                        filtered.append(log_val)
                else:
                    self.transient_buffer[pt_type].append(log_val)
                    filtered.append(prev_val)

        if self.filtered_rates[pt_type]:
            current_log_rate = self.filtered_rates[pt_type][-1]
            predicted_rate = 1 / (2.71828 ** (-current_log_rate))
            self.predicted_arrival_rates[pt_type].append(predicted_rate)

    def _update_arrival_rate_and_booking_limits(self):
        for pt in self.prompt_types:
            pt_type = pt["type"]
            predictions = self.predicted_arrival_rates[pt_type]
            if predictions:
                pt["arrival_rate"] = mean(predictions)
    ...
    def add_request(self, request: Request) -> None:
        ...
        if hasattr(request, "prompt_type") and hasattr(request, "arrived_at"):
            pt_type = request.prompt_type
            if pt_type in self.arrival_logs:
                self.arrival_logs[pt_type].append(request.arrived_at)
                if len(self.arrival_logs[pt_type]) > 1:
                    self._predict_and_store_log_rate(pt_type)
