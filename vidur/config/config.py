...
class BaseExecutionTimePredictorConfig(BasePolyConfig):
    compute_input_file: str = field(
        default="./data/profiling/compute/a100/meta-llama/Meta-Llama-3-8B/mlp.csv",
        metadata={"help": "Path to the compute input file."},
    )
    attention_input_file: str = field(
        default="./data/profiling/compute/a100/meta-llama/Meta-Llama-3-8B/attention.csv",
        metadata={"help": "Path to the attention input file."},
    )
    all_reduce_input_file: str = field(
        default="./data/profiling/network/a100_pairwise_nvlink/all_reduce.csv",
        metadata={"help": "Path to the all reduce input file."},
    )
    send_recv_input_file: str = field(
        default="./data/profiling/network/a100_pairwise_nvlink/send_recv.csv",
...
class SimulationConfig(ABC):
    ...
    def create_from_cli_args(cls):
        ...
        print("Simulation config create from cli args")
...
@dataclass
class ModifiedGeneralNestedBookingLimitSchedulerConfig(BaseReplicaSchedulerConfig):
    # 如果需要其他字段，可在此处添加
    # 例如：
    # some_parameter: int = field(default=123, metadata={"help": "Custom param"})
    total_limit: int = field(
    ...
    @staticmethod
    def get_type():
        return ReplicaSchedulerType.MODIFIED_GENERAL_NESTED_BOOKING_LIMIT
    
@dataclass
class ArrivalRateUpdateGeneralNestedBookingLimitSchedulerConfig(BaseReplicaSchedulerConfig):
    # 如果需要其他字段，可在此处添加
    # 例如：
    # some_parameter: int = field(default=123, metadata={"help": "Custom param"})
    total_limit: int = field(
    ...
    @staticmethod
    def get_type():
        return ReplicaSchedulerType.ARRIVAL_RATE_UPDATE_GENERAL_NESTED_BOOKING_LIMIT
...
@dataclass
class OurModifiedBookingLimitSchedulerConfig(BaseReplicaSchedulerConfig):
    # 如果需要其他字段，可在此处添加
    # 例如：
    # some_parameter: int = field(default=123, metadata={"help": "Custom param"})
    total_limit: int = field(
    ...
    @staticmethod
    def get_type():
        return ReplicaSchedulerType.OUR_MODIFIED_BOOKING_LIMIT